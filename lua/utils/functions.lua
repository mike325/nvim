...
function M.render_uml(fname)
    local executable = require('utils.files').executable

    local cmd = {}
    if executable 'plantuml' then
        table.insert(cmd, 'plantuml')
    elseif executable 'java' and require('utils.files').is_file(vim.fn.stdpath 'state' .. '/utils/plantuml.jar') then
        local jar_path = vim.fn.stdpath 'state' .. '/utils/plantuml.jar'
        vim.list_extend(cmd, { 'java', '-jar', jar_path })
    else
        vim.notify('Missing plantuml cmd', vim.log.levels.ERROR, { title = 'PlantUML' })
        return false
    end
    ...
    require('utils.async').makeprg {
        ...
        jump = false,
        dump = true,
        callbacks = function(_)
            local img = fname:gsub('%.puml$', '.png')
            vim.ui.open(img)
        end,
    }
end
